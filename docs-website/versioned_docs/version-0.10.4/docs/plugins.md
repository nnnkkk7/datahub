...
If you have any custom Authentication or Authorization plugin define in `authorization` or `authentication` section of [application.yml](https://github.com/datahub-project/datahub/blob/master/metadata-service/factories/src/main/resources/application.yml) then migrate them as per below steps.
...
2. Install Plugin: Install the plugins as per steps mentioned in [Plugin Installation](#plugin-installation). Here you need to map the configuration from [application.yml](https://github.com/datahub-project/datahub/blob/master/metadata-service/factories/src/main/resources/application.yml) to configuration in `config.yml`. This mapping from `application.yml` to `config.yml` is described below
